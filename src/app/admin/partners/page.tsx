...
export default function AdminPartnersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const partnerToSave: AdminPartner = {
        ...form,
        name: form.name.trim(),
        ...
        <div className="mx-auto max-w-7xl space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h1 className="mt-1 text-3xl font-semibold">Partners</h1>
              <p className="mt-1 text-gray-500">Manage partner companies</p>
            </div>
            <button
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
              {filteredPartners.map((partner) => (
                <div
                  key={partner.id}
                  className="group relative rounded-lg border border-gray-800 bg-gray-900/50 p-4 transition-colors hover:border-gray-700"
                >
                  ...
                  <div className="text-center">
                    <h3 className="mb-1 line-clamp-1 text-xs font-medium text-white">
                      {partner.name}
                    </h3>
                    {statusBadge(partner.status)}
                  </div>
                  <div className="absolute inset-0 flex items-center justify-center gap-2 rounded-lg bg-[#1b1b1b]/80 opacity-0 transition-opacity group-hover:opacity-100">
                    <button
                      onClick={() => startEdit(partner)}
                      className="rounded-md bg-[#43b14b] px-2 py-1 text-xs font-medium text-white hover:bg-[#3a9a41]"
                    >
                      Edit
                    </button>
                    <button
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1b1b1b]/70 px-4 backdrop-blur-sm">
              <div className="w-full max-w-2xl rounded-2xl border border-gray-800 bg-gray-900 p-6 shadow-2xl">
                <div className="mb-6 flex items-center justify-between">
                  <div>
                    <p className="text-sm uppercase tracking-wider text-gray-400">
                      {form.name ? 'Edit Partner' : 'New Partner'}
                    </p>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {form.logo && (
                        <div className="relative h-16 w-32 overflow-hidden rounded-md bg-gray-800">
                          <Image
                            src={form.logo}
                            alt="Logo"
                            fill
